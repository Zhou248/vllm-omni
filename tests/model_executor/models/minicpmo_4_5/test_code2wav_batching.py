...
def test_estimator_timestep_embeddings_are_precomputed_per_cfm_call():
    ...
    # Each eager Euler step keeps its original 2 * batch CFG call shape, but the
    # host-to-device frequency construction stays outside graph capture.
...
def test_npu_graph_dispatch_captures_and_replays_exact_shape_buckets(monkeypatch):
    ...
    # HiFT contains random phase generation and must stay outside graph capture.
